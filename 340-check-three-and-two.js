// Codewars Challenge 2024
// 340/366
// https://www.codewars.com/kata/5a9e86705ee396d6be000091/train/javascript

/*
Given an array with exactly 5 strings "a", "b" or "c" (chars in Java, characters in Fortran, Chars in Haskell), check if the array contains three and two of the same values.

Examples
["a", "a", "a", "b", "b"] ==> true  // 3x "a" and 2x "b"
["a", "b", "c", "b", "c"] ==> false // 1x "a", 2x "b" and 2x "c"
["a", "a", "a", "a", "a"] ==> false // 5x "a"
*/

function checkThreeAndTwo(array) {
  const counts = {};

  for (let i = 0; i < array.length; i++) {
    const key = array[i]; // this can be 'a', 'b' or 'c'
    if (Object.hasOwn(counts, key)) {
      counts[key] += 1; // if property exist, add 1
    } else {
      counts[key] = 1; // if property doesn't exist, initialized with value 1
    }
  }

  let values = Object.values(counts); // take value of object counts as array
  return values.includes(3) && values.includes(2);
}

console.log(checkThreeAndTwo(["a", "a", "a", "b", "b"]));
console.log(checkThreeAndTwo(["a", "c", "a", "c", "b"]));
console.log(checkThreeAndTwo(["a", "a", "a", "a", "a"]));
console.log(checkThreeAndTwo(["b", "b", "b", "b", "c"]));
console.log(checkThreeAndTwo(["c", "a", "a", "a", "b"]));
